...
public class LocalDatabase extends SQLiteOpenHelper {
    ...
    {
        super(context, "BookmarkMyDatabase.db", null, 1);
    }
   ...
   String tableBookmark="create table bookmark(id integer primary key   ,title text,summary text,content text,authorNom text)";
    ...
    public Boolean checkBookmarkExInDb(String id){

        String fetchdata="SELECT id  FROM bookmark WHERE  id="+id;
        SQLiteDatabase sqLiteDatabase=this.getReadableDatabase();

        Cursor cursor=sqLiteDatabase.rawQuery(fetchdata, null);
        Log.d("cursor countdb : ",cursor.getCount()+"");
